    ...
    <div className="relative flex items-center justify-center w-full h-[2vw] mt-5 p-2 text-white">
      ...
      {query.length > 0 && (
        <div className="absolute w-[34rem] text-white h-[50vh] top-[90%] z-10 mt-5 overflow-auto">
          {searchMovie.map((d, i) => (
            <Link to={`/${d.media_type || title}/details/${d.id} `} key={i}  className=" rounded  p-10  bg-[#0f0b20] gap-5  hover:bg-[#6556cd] duration-300 font-semibold w-full flex justify-start items-center border-b-2  border-zinc-500" >
              
              <img className=" z-10 w-[10vw] h-[7vw] bg-cover "
                src={d.backdrop_path || d.poster_path || d.file_path  ? `https://image.tmdb.org/t/p/original/${d.backdrop_path || d.poster_path || d.file_path}`
